...
export default function HomePage() {
  ...
  const cacheRef = useRef<Map<string, { ts: number; items: Suggestion[] }>>(new Map())

  const formatFullAddress = (s: Suggestion) => {
    const b = s.borough ? `${s.borough}, ` : ''
    const z = s.zipcode ? ` ${s.zipcode}` : ''
    return `${s.address}, ${b}NY${z}`.replace(/\s+/g, ' ').trim()
  }
  ...
  useEffect(() => {
    ...
    // If we have a fresh cached result, show it immediately (perceived speed win).
    const cacheKey = query.trim().toLowerCase()
    const cached = cacheRef.current.get(cacheKey)
    const now = Date.now()
    if (cached && now - cached.ts < 5 * 60 * 1000) {
      setSuggestions(cached.items)
      setShowDropdown(cached.items.length > 0)
    }
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        if (data.suggestions?.length) {
          ...
          cacheRef.current.set(cacheKey, { ts: Date.now(), items: data.suggestions })
        } else {
          setSuggestions([])
          setShowDropdown(false)
          cacheRef.current.set(cacheKey, { ts: Date.now(), items: [] })
        }
      } catch (e) {
  ...
  // Select a suggestion but don't navigate immediately.
  // This lets the user see/confirm the full formatted address in the input first.
  const handlePickSuggestion = (s: Suggestion) => {
    setSelectedSuggestion(s)
    setQuery(formatFullAddress(s))
    setShowDropdown(false)
    setSelectedIndex(-1)
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // If the user picked a suggestion, go directly to the BBL.
    if (selectedSuggestion?.bbl) {
      router.push(`/building/${selectedSuggestion.bbl}`)
      return
    }
            ...
            <div className="relative max-w-2xl mx-auto" ref={dropdownRef}>
                ...
                <div className="relative group">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      disabled={loading}
                    />
                    {suggestLoading && !loading && query.length >= 2 && (
              ...
              {selectedSuggestion && (
                <div className="mt-2 text-sm text-[#94a3b8]">
                  Selected: <span className="text-white/90">{formatFullAddress(selectedSuggestion)}</span> • press “View” or Enter to open.
                </div>
              )}
